...
<script type="text/javascript" src="/assets/webuploader/webuploader.min.js"></script>
...
<script>
    ...
    uploader.on( 'fileQueued', function( file ) {
        var $li = $(
            ...
            '<img>' +
            '<div class="imageName">' + file.name + '</div>' +
            '</div>'
            ),
            $img = $li.find('img');
        // $list为容器jQuery实例
    ...
    uploader.on( 'uploadSuccess', function( file ) {
        $( '#'+file.id ).append('<div class="success-info">上传成功</div>');
    });
    // 文件上传失败，显示上传出错。
    uploader.on( 'uploadError', function( file ) {
        ...
        // 避免重复创建
        if ( !$error.length ) {
            $error = $('<div class="error-info"></div>').appendTo( $li );
        }
        $error.text('上传失败');
    });
